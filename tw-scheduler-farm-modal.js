(function () {
  ...
  // ═════════════════════════════════════════════════════════
  // ✅ #7 VERIFICAÇÃO DE FARMS ATRASADOS (SEM DISPARO AUTOMÁTICO)
  // ═════════════════════════════════════════════════════════
  ...
  function verificarFarmsAtrasados() {
      ...
      farms.forEach(farm => {
          ...
          try {
              ...
              // ✅ APENAS DETECTA E REGISTRA O ATRASO (SEM EXECUTAR)
              if (nextRun && nextRun < now) {
                  atrasosDetectados++;
                  
                  // ✅ Log informativo (sem executar)
                  if (atrasosDetectados <= 3) { // Limitar logs para não poluir console
                      console.log(`[Farm] ⏰ Farm atrasado: ${farm.origem} → ${farm.alvo} (${Math.floor((now - nextRun) / 60000)} min atrás)`);
                  }
                  
                  // ✅ Marcar como necessitando recalculo
                  if (!agendamentoBase.status || !agendamentoBase.status.includes('Atrasado')) {
                      ...
                      FarmLogger.log('DELAYED_DETECTED', farm, { 
                          nextRun: farm.nextRun,
      ...
      if (atrasosDetectados > 0) {
          setList(lista); // Salvar status atualizados
          
          // ✅ Atualizar UI se houver muitos atrasos
          if (atrasosDetectados > 5 && document.getElementById('farm-list-container')) {
              document.getElementById('farm-list-container').innerHTML = renderFarmList();
          }
      }
  }
  ...
  function startFarmMonitor() {
    ...
    setInterval(verificarFarmsAtrasados, 15000); // ✅ AGORA SEM DISPARO AUTOMÁTICO
  ...
  function init() {
    ...
    console.log('[TW Farm Inteligente] ✅ Carregado v2.2 - Sem disparo automático de farms atrasados!');
